...
@test "reject because name is on deny list" {
  run kwctl run annotated-policy.wasm -r test_data/namespace.json --settings-json '{"denied_names": ["foo", "test-ns"]}'
...
@test "accept because name is not on the deny list" {
  run kwctl run annotated-policy.wasm -r test_data/namespace.json --settings-json '{"denied_names": ["foo"]}'
  # this prints the output when one the checks below fails
  echo "output = ${output}"
